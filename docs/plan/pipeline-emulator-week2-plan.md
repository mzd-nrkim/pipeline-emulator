...
- [ ] T1-1. FastAPI 라우터 구성 (`ui-backend/app/api/`)
  - [ ] `/stages` 라우터 — 단계별 문서 수·상태·마지막 처리 시각
  - [ ] `/runs` 라우터 — DAG 실행 회차 목록·상태·소요시간
  - [ ] `/executions` 라우터 — 단계별 실행 결과 (run_id 기준)
- [ ] T1-2. Airflow REST API 연동 모듈 (`ui-backend/app/services/airflow.py`)
  - [ ] DAG 실행 현황 조회 (run 목록·상태·소요시간)
  - [ ] MVP T7 Airflow REST 연동 로직 이관
- [ ] T1-3. MySQL 집계 쿼리 모듈 (`ui-backend/app/services/mysql_aggregator.py`)
  - [ ] 단계별 문서 수(입력/출력) 집계
  - [ ] 최근 처리 시각, 실패 사유 조회
  - [ ] MVP T7 MySQL 카운트 쿼리 이관
- [ ] T1-4. PII 마스킹 현황 집계 모듈 (`ui-backend/app/services/pii_stats.py`)
  - [ ] 마스킹/비마스킹 문서 수 집계
...
- [ ] T1-6. **SSE 엔드포인트** (`/sse/stages`) 구현
  - [ ] `asyncio` 기반 SSE 응답 스트리밍 (lodestar `subscribeBridgeStatus` 패턴 참조)
  - [ ] 단계 카운트 갱신 이벤트 emit (폴링 주기 설정)
...
- [ ] **@xyflow/svelte 읽기전용 단계 그래프** (실물 확인 후 결정 — 현재 CSS Grid 기반 StageNode로 대체됨)
  - [ ] Bronze/Silver/Gold 3레이어 고정 레이아웃, DAG 6개 노드
  - [ ] 엣지 방향 연결, 확대·축소·이동
  - (현재 CSS Grid 기반 StageNode 구현 실물 확인 → xyflow 도입 필요 여부 결정 후 구현)
...
- [ ] SSE 실시간 갱신 연동 (별도 새로고침 없이 반영), 모바일 우선 레이아웃 _(SSE 미연결 — mock 데이터 사용 중)_
  - [ ] `EventSource` 클라이언트 구현 (lodestar `frontend/src/lib/api.ts` `subscribeBridgeStatus` 패턴 참조)
  - [ ] SSE 이벤트 수신 시 단계 카운트·상태 자동 갱신 (T1-6 SSE 엔드포인트 완료 후 연동)
...
- [ ] feature-flag 뼈대 — 환경변수 + Docker Compose profile 스위치를 화면에 노출·적용 _(UI 토글만 구현, 실제 백엔드 반영 없음)_
  - [ ] 현재 feature-flag 상태 읽기 API 엔드포인트 구현 (`/config` 또는 `/flags`)
  - [ ] 토글 변경 UI 반응 — 변경 시 "재시작 필요" 안내 표시 (백엔드 즉시 반영은 Week 2 범위 밖)

### T4. 데모 시나리오 & 발표 자료

- [ ] T4-1. 데모 시나리오 문서 작성 (`docs/demo-scenario.md`)
  - [ ] 투입 → Bronze → Silver → Gold staged 단계별 시연 흐름 기술
  - [ ] 각 단계에서 대시보드에서 확인할 내용 명시 (노드 상태·카운트·세부 패널)
- [ ] T4-2. 설정 메뉴 로드맵 스토리 추가 (T4-1 문서에 섹션 추가)
  - [ ] 미구현 축(검색서빙·Presidio·분산실행·ES클러스터)의 "다음 계획" 배지 시나리오 포함
- [ ] T4-3. 발표 자료 준비 (경영진·고객·팀 시연용)
  - [ ] 흐름 다이어그램 + 주요 수치 (Week 2 성과)
  - [ ] 아키텍처 설명 슬라이드
