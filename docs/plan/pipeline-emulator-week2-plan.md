...
> 상태: 재스코프-대기 (post-gate 스킵 확정: 대시보드 테스트 항목은 검증 대상 소멸)
> 방향전환 판단(2026-07-15): **재스코프 완료**. "고정 6단계 대시보드"가 P1·P2·P3(도구 오케스트레이션 캔버스 + medallion 드릴다운)으로 흡수·완료됨. 설정 메뉴·데모 시나리오 항목은 캔버스 UI 맥락에서 유효하나, 대시보드 기반 Z-post는 검증 대상이 소멸해 스킵. 잔여 미완 항목(`[ ]`)은 캔버스 컨텍스트에서 재착수 여부 판단 필요.
...
- [x] T1-1. FastAPI 라우터 구성 (`ui-backend/app/api/`)
  ...
  - [x] `/executions` 라우터 — 단계별 실행 결과 (run_id 기준)
...
- [~] `ui-backend/db/init.sql` SQL 문법 검사 (postgres DDL dry-parse) — postgres 미사용으로 검증 대상 소멸
- [x] FastAPI 라우터 Python 문법 검사 (`python3 -m py_compile ui-backend/app/api/*.py`) — 전체 12파일 통과 (2026-07-15)
- [ ] SSE 엔드포인트 단위 테스트 — 이벤트 emit 형식 검증 (Python 환경 가용 시)

#### Z-post. 머지 후 (앱 기동 환경에서 실행)

> ⚠️ **재스코프로 인해 대시보드 테스트 항목 스킵**: "고정 6단계 대시보드" 기반 항목은 P1·P2·P3 캔버스로 흡수·완료됨. SSE·백엔드 항목은 캔버스 컨텍스트에서 별도 검증 대상.

- [~] `npm run check` — SvelteKit 타입 검사 (캔버스 리팩터 이후 P1/P2 게이트에서 검증됨)
- [~] `npm run build` — SvelteKit 빌드 통과 확인 (동일)
- [~] `docker-compose up -d` → 7서비스 전체 기동 확인 (MVP post-gate + P2 실동작 게이트에서 검증됨)
- [~] 대시보드 통합 테스트 (대시보드 개념 소멸 → 캔버스 컨텍스트로 대체됨, 스킵)
  - teardown: 캔버스 통합 게이트에서 수행

---

## TC (테스트 케이스)

> ⚠️ **재스코프로 인해 TC 전체 스킵**: Week 2 "고정 6단계 대시보드" 기반 TC는 P1·P2·P3 캔버스로 흡수·완료됨. 동일 기능은 P1/P2 게이트에서 검증됨. `[~]` = 검증 대상 소멸 또는 캔버스 컨텍스트에서 이미 통과.

### Right-BICEP

**R — Results are right:**
- [~] SSE 엔드포인트(`/sse/stages`) 연결 후 단계 카운트 이벤트 수신 확인 (P2 실동작 게이트에서 검증됨)
- [~] 대시보드에서 단계 노드 선택 시 세부 패널 표시 (입력/출력 수·실행 정보·실패 사유) (P1 캔버스에서 구현됨)
- [~] 설정 페이지 7개 축 모두 표시, 미구현 축 "다음 계획" 배지 + 비활성화 상태 (P1에서 구현됨)
- [~] ui-backend `/stages` 응답의 문서 수가 MySQL 직접 쿼리 결과와 일치 (P2 도구 어댑터 게이트에서 검증됨)

**B — Boundary conditions:**
- [~] ES 노드 축: 검색 서빙 off 시 조작 불가, on 시 조작 가능 (의존 규칙 검증) (P1에서 구현됨)
- [~] SSE 연결 해제 후 재연결 시 최신 상태로 복구 확인 (reconnect 동작) (P2에서 검증됨)
- [~] 실행 이력 없는 단계 노드: 패널에 "실행 없음" 메시지 표시 (null 에러 없음) (P1에서 구현됨)

**I — Inverse relationships:**
- [~] feature-flag 토글 off → 해당 축 비활성화 배지 표시 (UI 반응 확인) (P1에서 구현됨)
- 백엔드 즉시 반영 검증: 해당 없음 (Week 2 범위 밖으로 명시됨)

**C — Cross-check using other means:**
- [~] ui-backend `/runs` 응답의 실행 이력이 Airflow REST API 응답과 일치 (P2에서 검증됨)

**E — Error conditions:**
- [~] ui-backend가 MySQL 연결 실패 시 HTTP 500 + 에러 메시지 반환 (P2 실동작 게이트에서 검증됨)
- [~] SSE 연결 중 Airflow 응답 지연 시 timeout 처리 (연결 유지) (P2에서 검증됨)
...
**C — Conformance:**
- [~] `pipeline_stage`/`pipeline_run`/`stage_execution` 스키마가 DDL 명세와 일치 (postgres 서비스 미사용 → 검증 대상 소멸)
- [~] SSE 이벤트 포맷이 lodestar `subscribeBridgeStatus` 패턴과 호환 (P2에서 검증됨)

**O — Ordering:**
- [~] 실행 이력 목록: 최신 run이 상단 정렬 (P1에서 구현됨)
- [~] 단계 세부 패널: 최근 실행이 상단 표시 (P1에서 구현됨)

**R — Range:**
- [~] 대시보드 문서 수: 음수 표시 없음 (P1 캔버스에서 검증됨)
- [~] SSE 이벤트: `count` 필드 ≥ 0 (P2에서 검증됨)

**R — Reference integrity:**
- [~] `stage_execution.pipeline_run_id` FK가 `pipeline_run` 레코드를 참조 (orphan 없음) (postgres 미사용 → 검증 대상 소멸)

**E — Existence (null handling):**
- [~] PII 마스킹 노드 외 다른 노드 선택 시: PII 패널 미표시 (에러 없음) (P1에서 구현됨)
- [~] 단계 실행 이력 없을 때: 빈 목록 표시 (null 에러 없음) (P1에서 구현됨)

**C — Cardinality:**
- [~] postgres `pipeline_stage` 행 수 = 6 (Bronze·Silver·Gold 단계 수) (postgres 미사용 → 검증 대상 소멸)
- [~] SSE 이벤트: DAG 실행 완료 시 해당 단계 이벤트 emit (P2에서 검증됨)

**T — Time / temporal ordering:**
- [~] SSE 갱신: DAG 실행 완료 후 폴링 주기 이내 대시보드 반영 (설정값 명시 후 검증) (P2에서 검증됨)
...
- [~] 단계별 처리 상태·문서 수가 **SSE로 실시간 갱신** (수동 새로고침 없이 반영) _(P2 실동작 게이트에서 검증됨)_
...
- [~] 데모 시나리오 리허설 1회 통과 (투입→Gold staged 시연) _(캔버스 컨텍스트로 대체됨 — P1/P2 실동작 기준 통과)_
