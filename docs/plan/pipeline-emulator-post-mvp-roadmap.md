...
> 선행: [pipeline-emulator-mvp-plan.md](./done/pipeline-emulator-mvp-plan.md) · [pipeline-emulator-week2-plan.md](./done/pipeline-emulator-week2-plan.md) *(둘 다 완료·done/ 이동)*
> 근거: [pipeline-emulator-decisions.md](../pipeline-emulator-decisions.md) "다음 계획"
>
> **캔버스 방향 인덱스(2026-07-15 추가 / 2026-07-21 갱신)**: Week 2 "고정 6단계 대시보드" → 도구 오케스트레이션 캔버스로 전환. P1·P2·P3 완료. 캔버스 기반 신규 feat 계획 3건도 **전부 완료(done/ 이동)** — §캔버스 리팩터·§캔버스 기반 feat 참조. **F1~F6 전부 완료, 잔여는 F7(트리거 대기)뿐.**
...
| # | 기능 축 | 토글 | 전환 트리거 | 우선순위 | 상태 | 계획서 |
|---|---------|------|-------------|:-------:|:----:|--------|
| F1 | 검색 서빙 (ES) | `SEARCH=off\|lite\|hybrid` | 검색이 데모 범위로 들어올 때 (데모 클라이맥스) | ★★★ | ✅ 완료 | [feat-f1](./done/pipeline-emulator-feat-f1-search-es.md) |
| F2 | 실시간 CDC | `CDC=off\|on` | "소스 변경 → 실시간 반영" 시연 요구 | ★★★ | ✅ 완료 | [feat-f2](./done/pipeline-emulator-feat-f2-realtime-cdc.md) |
| F3 | NiFi 수집기 | `COLLECTOR=script\|nifi` | 수집 흐름 자체가 시연 대상이 될 때 | ★★ | ✅ 완료 | [feat-f3](./done/pipeline-emulator-feat-f3-nifi-collector.md) |
| F4 | 청킹·엔리치 실 API | `ENRICH=mock\|real` | 고객사 API 실제 오픈 | ★★ | ✅ 완료 | [feat-f4](./done/pipeline-emulator-feat-f4-real-enrich-api.md) |
| F5 | 풀 Presidio 2-Layer | `MASK=regex\|presidio` | 마스킹 정밀도가 시연 포인트가 될 때 | ★★ | ✅ 완료 | [feat-f5](./done/pipeline-emulator-feat-f5-presidio-2layer.md) |
| F6 | CeleryExecutor 분산 | `PROFILE=local\|celery` | "실제 분산 워커 동작"이 요구될 때 | ★ | ✅ 완료 | [feat-f6](./done/pipeline-emulator-feat-f6-celery-executor.md) |
| F7 | ES 다중 노드 | `ES=single\|cluster` | 고가용성/부하가 시연 대상 (F1 on 전제) | ★ | ⏳ 대기(트리거) | [feat-f7](./idea/pipeline-emulator-feat-f7-es-cluster.md) |
...
## 캔버스 기반 feat (완료 — done/ 이동)

P1·P2·P3 캔버스 기반에서 파생된 신규 기능 계획. F1~F7 feature-flag 축과 독립적. **3건 전부 완료·done/ 이동.**

| 계획서 | 상태 | 요약 |
|--------|------|------|
| [feat-infra-view-buildout](./done/pipeline-emulator-feat-infra-view-buildout.md) | ✅ 완료 | 인프라 연결 뷰 실체화 (스텁 3엣지 → 유의미한 의존 그래프) |
| [feat-node-config-persistence](./done/pipeline-emulator-feat-node-config-persistence.md) | ✅ 완료 | 노드 설정 저장 + 도구 API 전수조사·기능 리스트업 |
| [feat-run-history-detail](./done/pipeline-emulator-feat-run-history-detail.md) | ✅ 완료 | 실행 이력 상세화 (성공/실패/시간 → 스테이지별 근거) |
