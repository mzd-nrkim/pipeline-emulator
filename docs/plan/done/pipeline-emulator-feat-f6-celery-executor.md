...
- [x] 분산 워커가 태스크를 실행 (scheduler가 아닌 celery worker에서 처리 확인)
- [x] DAG 코드 변경 0으로 동일 결과 산출 (TaskFlow API)
- [x] `PROFILE=local`로 즉시 복귀 가능
...
- [x] 기동: `AIRFLOW_EXECUTOR=CeleryExecutor AIRFLOW_SQL_CONN=mysql+pymysql://emulator:emulator_pass@mysql:3306/pipeline_emulator CELERY_BROKER_URL=redis://valkey:6379/1 CELERY_RESULT_BACKEND=db+mysql+pymysql://emulator:emulator_pass@mysql:3306/pipeline_emulator COMPOSE_PROFILES=celery docker compose up -d`
- [x] 존재/기동 확인(Existence): `docker compose ps` — `valkey`·`airflow-worker` 컨테이너 `Up`, `airflow` healthy
- [x] executor 확인(Right): `docker compose exec airflow airflow config get-value core executor` == `CeleryExecutor`
- [x] 메타DB 확인: `docker compose exec airflow airflow config get-value database sql_alchemy_conn` == MySQL URL
- [x] DAG 트리거: `docker compose exec airflow airflow dags trigger gold_6_es_indexing --run-id celery_worker_verify_...`
- [x] **워커 실행 증명(Right — 3중)**: worker 로그 `Executing command in Celery`·`Running ... on host 10045dc1112e` 확인 + Task Instance 4건 모두 hostname=`10045dc1112e`(airflow-worker) + scheduler 로그 dispatch-only(`Adding to queue`) 대조
- [x] 결과 교차검증(Cross-check): DAG 성공 & gold_staged_documents 15건 모두 indexed + ES pdis_cft 15건 색인 일치
- [x] **가역성 검증(Inverse — 핵심)**: celery down → `docker compose up -d` → `airflow-worker`·`valkey` 부재, executor==`SequentialExecutor`
- [x] **teardown(필수)**: `COMPOSE_PROFILES=celery docker compose down -v` — 전 볼륨 삭제 완료
