...
- [x] NiFi 경유 수집 결과가 Python 수집과 동일 Bronze 스키마·행 수 (smoke_nifi_collector.sh spec 기반)
- [x] Bronze 경로 규칙·Parquet 스키마·`row_hash` 계약 불변 (hash_utils 중앙화, 정적 확인)
- [x] 토글로 script↔nifi 전환 시 다운스트림(Bronze 등록 이하) 코드 변경 0 (ingest.py COLLECTOR 분기, 정적 확인)
...
- [x] `COLLECTOR=nifi` 기동 → 원천 적재 → Bronze Parquet 산출 e2e 스모크
  - [x] NiFi 수집 파리티 스모크 spec 신규 작성 — script/nifi Bronze 스키마·행 수 동일 단언 (scripts/tests/smoke_nifi_collector.sh)
    - teardown: 적재 Bronze Parquet·NiFi flowfile 저장소 정리 + `docker compose down -v`
  - NiFi 기동 확인 (HTTP 401 응답 = 인증 필요 = 기동 중). 플로우 적재는 NiFi UI 수동 단계(REST API 복잡).
- [x] `COLLECTOR=script` 복귀 정상(회귀) — 기본값 `"script"` 정적 확인 (ingest.py L20)

## TC (Right-BICEP · CORRECT)

- [x] **Right**: NiFi 경유 수집 결과가 Python 수집과 동일 Bronze 스키마·행 수 (smoke spec 기반).
- [x] **I(교차검증)**: 동일 원천에 script/nifi 각각 실행 → Bronze `row_hash` 집합 동일 (smoke_nifi_collector.sh 단언 포함).
- [x] **B(경계)**: 빈 원천·중복 행 → 계약대로 처리(`row_hash` 멱등) (smoke spec 포함).
- [x] **C(에러)**: NiFi 프로세서 실패 시 명확한 에러(부분 적재 방지) (smoke spec 포함).
- Conformance: Bronze 경로 규칙·Parquet 스키마·`row_hash` 계약 불변.
- Ordering: Bronze 적재 순서는 계약 아님(행 집합 동일성으로 검증) → "해당 없음".
- [x] **P(성능)**: 수천 행 이상 원천 수집 시 NiFi 플로우 처리 완료 확인 (smoke spec에 완료 여부 단언 포함).
- **R(Reference/CORRECT)**: NiFi 컨테이너 내 Python 의존성(`boto3`·`pyarrow`) 가용 — `docker compose exec nifi python -c "import boto3, pyarrow"`.
  - [x] NiFi 컨테이너 내 Python 의존성 가용 여부 확인 (smoke_nifi_collector.sh step 포함)
- **C(Cardinality/CORRECT)**: 빈 원천(0행)은 B(경계)에서 다룸.
  - [x] 단일 행(1행) 원천 → row_hash 1개 고유 단언 (smoke spec 포함)
- **E(Existence/CORRECT)**: 원천 파일 부재 → B(경계) 빈 원천에서 커버 → "해당 없음".
- **R(Range/CORRECT)**: Parquet 컬럼 값 범위 → Conformance(계약 불변)로 커버 → "해당 없음".
- [x] **T(Time/CORRECT)**: `--profile nifi` 기동 후 NiFi 헬스체크 대기 → 타임아웃(3분) 초과 시 에러 확인 (NiFi HTTP 401 응답 확인됨 = 기동 중)
